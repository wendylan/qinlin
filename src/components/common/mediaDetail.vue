				...
				<div class="ad_mediaInfoPanel" v-model="obj">
					<el-row type="flex" class="row-bg">
						...
						<el-col :span="8">
							...
							<li>楼盘价格：<span>{{obj.cType}}</span></li>
...
export default {
	...
	methods:{
		getDataFun(){
			...
			//  console.log('res.data',res.data)SetMediaInfo
				this.obj.cType = res.data.cType
				this.obj.housePrice = res.data.cType
				...
				if(res.data.rID.toString().indexOf('440') > 0){
					this.obj.city = '广州'
				}else if(res.data.rID.toString().indexOf('106') > 0){
