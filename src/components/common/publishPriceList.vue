    ...
    <div class="mediaList_wrap">
      ...
      <div class="mediaList_container">
        <div class="table_wrap">
          <el-table border :data="publishPriceList" style="width: 100%" v-loading="loading" element-loading-text="拼命加载中"
                    element-loading-spinner="el-icon-loading">
            <el-table-column prop="rName" label="城市" min-width="31.3%" :filters="filtCity" :filter-method="filterCity"
              ...
              <template slot-scope="scope">
                ...
                <el-cascader v-if="scope.row.showInput" :options="allProvince" v-model="scope.row.cityArr" separator="-"
                             :show-all-levels="false" @change="seleProClient(scope.row)"
            ...
            <el-table-column class="tar" prop="publicPrice" label="刊例价" min-width="34.3%">
              <template slot-scope="scope">
                ...
                <el-input type="number" v-model="scope.row.priceUpdate" placeholder="请输入刊例价" class=" tar"
                          v-if="scope.row.showInput"></el-input>
              </template>
            </el-table-column>
            <el-table-column v-if="role=='SM'" label="操作" min-width="22.6%">
              <template slot-scope="scope">
                ...
                <div v-if="scope.row.changeBtn">
                  <el-button type="text" size="small" class="savePP"
                             @click.native.prevent="savePP(scope.row, scope.$index)">保存
                  </el-button>
                  <el-button type="text" size="small" class="cancelPP"
                             @click.native.prevent="cancelPP(scope.row, scope.$index)">取消
                  </el-button>
                </div>
              </template>
            </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      addPriceFun() {
        ...
        this.publishPriceList.unshift(obj);
      },
      ...
      seleProClient(rows) {
        ...
        this.$set(
          rows,
          "rNameUpdate",
          areaToText.toTextCity(rows.cityUpdate)
        );
      },
      //删除
...
<style scoped>
  ...
  /deep/ .el-table-filter__list::-webkit-scrollbar-thumb {
    /*滚动条里面小方块*/
