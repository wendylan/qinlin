			...
			<div class="mediaMana_content_top">
				<div class="content_top_wrap">
					<div class="title">
						...
						<h3>{{downReport.apName}} &nbsp;&nbsp;{{formatTime(downReport.apcTime)}}</h3>
					</div>
					<div class="detail">
						<p>
							<em>公司名称：</em><i>{{downReport.cName}}</i>
							<em>投放城市：</em><i>{{downReport.rIDs}}</i>
							<em>投放面数：</em><i>{{downReport.totalNum}}</i>
						</p>
					</div>
				</div>
			</div>
...
export default {
	...
	methods: {
		...
		getInitData(){
			...
			// 下刊数据(组合图片)
			tempADList = this.constructImg(tempADList, downImginfo);
			this.downReportArr = tempADList;
			console.log('downimginfo', this.downReportArr);
			// 下刊数据(组合图片按图片分)
			this.imgInfo = this.initImg(tempADList, downImginfo);
			...
			// 			console.log('downimginfo', this.downReportArr);
			// 			// 下刊数据(组合图片按图片分)
			// 			this.imgInfo = this.initImg(resArr, downImginfo);
			// 		}).catch(res =>{
			// 			console.log(res);
			// 		});
			// 	}else{
...
<style scoped>
  ...
  /deep/ .el-table_1_column_8, /deep/ .el-table_1_column_9, /deep/ .el-table_1_column_10 {
